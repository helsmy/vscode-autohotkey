...
class Beautifier {
    ...
    constructor(js_source_text: string, options: FormatOptions) {
        ...
        let acorn = {
            "isIdentifierChar": function (code: number) {
            ...
            "isIdentifierStart": function (code: number) {
        ...
        line_starters = 'continue,try,throw,return,global,local,static,if,switch,case,default,for,while,loop,break'.split(',');
        reserved_words = line_starters.concat(['in', 'else', 'until', 'get', 'set', 'catch', 'finally', 'and', 'or']);
        following_bracket = false, begin_line = true;
        ...
        this.beautify = function () {
            ...
            while (true) {
                t = get_next_token();
                token_text = t[0], token_text_low = token_text.toLowerCase();
                token_type = t[1];
                ...
                if (token_type !== 'TK_INLINE_COMMENT' && token_type !== 'TK_COMMENT' &&
                    ...
                    if (!following_bracket && token_type === 'TK_RESERVED' && in_array(token_text_low, ['if', 'for', 'while', 'loop'])) {
        ...
        function split_newlines(s: string) {
        ...
        function print_newline(force_newline = false, preserve_statement_flags = false) {
            output_space_before_token = false;
        ...
        function start_of_statement() {
            if ((last_type === 'TK_RESERVED' && in_array(flags.last_text.toLowerCase(), ['local', 'static', 'global']) && token_type === 'TK_WORD') ||
                (last_type === 'TK_RESERVED' && flags.last_text.match(/^loop$/i)) ||
                ...
                if (!start_of_object_property()) {
                    allow_wrap_or_preserved_newline(token_type === 'TK_RESERVED' && in_array(token_text_low, ['loop', 'for', 'if', 'while']));
                }
        ...
        function all_lines_start_with(lines: string[], c: string) {
        ...
        function is_special_word(word: string) {
        ...
        function end_bracket_of_expression(pos: number) {
        ...
        function get_next_token() {
            ...
            if (begin_line) {
                ...
                if (line.indexOf('::') === -1) {
                    ...
                    if (m[3].trim().match(/^\{\s*(\s;.*)?$/)) {
                        parser_pos = last_LF + m[1].length + 1;
                        return [m[1], 'TK_WORD']
                    } else {
                        last_LF = next_LF, parser_pos = next_LF + 1, begin_line = true;
                        return [m[0], 'TK_UNKNOWN'];
                    }
                } else if (m = line.match(/^([~*]{0,2}((([<>]?[!+#^]){0,4}(`?[\x21-\x7E]|[a-z][a-z\d_]+))|(`?[\x21-\x7E]|[a-z][a-z\d_]+)\s+&\s+(`?[\x21-\x7E]|[a-z][a-z\d_]+))(\s+up)?::)(.*)$/i)) {
                    if (m[9].trim().match(/^\{\s*(\s;.*)?$/)) {
                        parser_pos = input.indexOf('::', parser_pos) + 2;
                        return [m[1].replace(/\s+/g, ' '), 'TK_WORD']
                    } else {
                        last_LF = next_LF, parser_pos = next_LF + 1, begin_line = true;
                        return [m[1].replace(/\s+/g, ' ') + m[9].trim(), 'TK_UNKNOWN'];
                    }
                }
            }
            ...
            if (acorn.isIdentifierChar(input.charCodeAt(parser_pos - 1))) {
                ...
                if (!(last_type === 'TK_DOT' ||
                    (last_type === 'TK_RESERVED' && in_array(flags.last_text.toLowerCase(), ['set', 'get'])))
                    && in_array(c.toLowerCase(), reserved_words)) {
            ...
            if (c === ';') {
                ...
                if (input.substring(last_LF + 1, parser_pos - 1).trim() === '') {
                ...
                return [comment.trimRight(), comment_type];
            }
            ...
            if (c === "'" || c === '"') { // string
                let sep = c, esc = false, f = parser_pos;
            ...
            if (c === '#') {
                ...
                let sharp = '#';
                c = input.charAt(parser_pos);
                if (parser_pos < input_length && c !== ':' && !in_array(c, whitespace)) {
                    while (parser_pos < input_length && (c = input.charAt(parser_pos)) !== ':' && !in_array(c, whitespace)) {
                        sharp += c;
                        parser_pos += 1;
                    }
        ...
        function handle_operator() {
            if (token_text === ':' && flags.ternary_depth === 0 && !flags.in_case) {
                ...
                if (output_lines.length > 0 && flags.last_text.match(/^[\u4e00-\u9fa5_a-zA-Z0-9]+$/)) {
                    let pos = input.indexOf('\n', last_LF + 1);
                    if ((new RegExp('^' + flags.last_text + ':(\\s*;.*)?$')).test(input.substring(last_LF + 1, pos === -1 ? input_length : pos).trim())) {
        ...
        function handle_unknown() {
            if (input_wanted_newline && (last_type === 'TK_UNKNOWN' || !just_added_newline())) {
