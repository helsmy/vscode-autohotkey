...
import { ScriptASTFinder, binarySearchIndex } from './scriptFinder';
...
export class TreeManager
{
    ...
    public docSymbolInfo(): SymbolInformation[] {
        const info = this.documentService.getDocumentInfo(this.currentDocUri);
        if (!info) return [];
    ...
    public getGlobalCompletion(): CompletionItem[] {
        ...
        if (!docinfo) return [];
        const symbols = docinfo.syntax.table.allSymbols();

        for (let [uri, symbols] of docinfo.syntax.table.includeSymbols()) {
            const base = URI.parse(docinfo.document.uri).fsPath;
    ...
    public getScopedCompletion(pos: Position): Maybe<CompletionItem[]> {
        ...
        const scope = this.getCurrentScope(pos, docinfo.syntax.table);
        const found = this.finder.find(docinfo.syntax.AST.script.stmts, pos, [Expr.Factor]);
    ...
    public getTokenAtPos(pos: Position): Maybe<Token> {
        ...
        if (!docinfo) return undefined;
        const tokens = docinfo.syntax.AST.script.tokens;
    ...
    public includeDirCompletion(position: Position): Maybe<CompletionItem[]> {
        ...
        const context = doc.LineTextToPosition(position);
